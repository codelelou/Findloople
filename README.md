...
* [お探しループゲームの作り方](#お探しループゲームの作り方)
  ...
  * [【ステップ1】 レベル作成（地形や建物などのマップ作成）](#ステップ1-レベル作成地形や建物などのマップ作成)
    * [まずはサンプルゲームをプレイしよう](#まずはサンプルゲームをプレイしよう)
    * [レベル作成の注意点（制約）](#レベル作成の注意点制約)
    * [レベル本体の編集](#レベル本体の編集)
  * [【ステップ2】 フラグリスト作成](#ステップ2-フラグリスト作成)
    * [フラグID](#フラグid)
    * [Tags](#tags)
    * [フラグIDの削除](#フラグidの削除)
  * [【ステップ3】 フラグ設定](#ステップ3-フラグ設定)
    * [Osgs_BP_FlagActorアクターをレベルに追加する](#osgs_bp_flagactorアクターをレベルに追加する)
    * [フラグのテスト](#フラグのテスト)
  * [仕上げ](#仕上げ)
    * [ゲーム情報の書き換え](#ゲーム情報の書き換え)
    * [Osgs_BP_FunctionLibrary](#osgs_bp_functionlibrary)
    * [インフォメーションの概要](#インフォメーションの概要)
* [パッケージ化（配布用ファイル作成）](#パッケージ化配布用ファイル作成)
* [テンプレートのアップデート](#注意テンプレートのアップデート中級者上級者向け)
* [応用編](#応用編)
  * [国際化（多言語対応）](#国際化多言語対応)
  * [8番ライクの作り方](#8番ライクの作り方)
    * [8番ライク向けにルール変更](#8番ライク向けにルール変更)
    * [写真撮影機能を無効化](#写真撮影機能を無効化)
    * [引き返したら異変を見つけたことにする](#引き返したら異変を見つけたことにする)
    * [8番ライク用にテキストを変更](#8番ライク用にテキストを変更)
    * [異変作りについて](#異変作りについて)
  * [メニューをカスタマイズ](#メニューをカスタマイズ)
  * [操作方法のキー設定](#操作方法のキー設定)
  * [プレイヤーのパラメーター](#プレイヤーのパラメーター)
  * [デカール（ゲーム内オリジナルポスターの作り方）](#デカールゲーム内オリジナルポスターの作り方)
  * [ゲームの画質について](#ゲームの画質について)
  * [サウンドの種類の設定方法](#サウンドの種類の設定方法)
  * [フォント](#フォント)
* [発展編](#発展編)
  * [トラップアクター](#トラップアクター)
  * [キャラクターにUE標準のThirdPersonを使用する（足音などキャラクターのカスタマイズ）](#キャラクターにue標準のthirdpersonを使用する)
  * [別プロジェクトに移行する](#別プロジェクトに移行する上級者向け)
* [ゲーム作成・異変作成時の注意点](#ゲーム作成異変作成時の注意点)
* [よくある質問](#よくある質問)
...
###### クリア後の見つかっていないモノの案内板
Osgs_Level_MainStageレベル上に配置しているOsgs_BP_Actor_NotFounds_3DTextアクターブループリント（\Content\Osagashi\Blueprint\Actor\NotFounds\Osgs_BP_Actor_NotFounds_3DText.uasset）で、クリア後の見つかっていないモノついての表示（「見つかっていない異変が8個あります」や「すべての異変が見つかりました」など）をしています。  

NotFoundsText変数で見つかっていないモノが1つ以上ある場合のテキストを、AllFoundsText変数で見つかっていないモノがない場合のテキストを設定できるようになっています。  

ShowActors配列変数は、見つかっていないモノの案内板を表示する時に一緒に表示するアクター（例えば案内板本体・枠など）を指定でき、クリア前はこの変数に指定したアクターも表示されません。  

HideTriggers配列変数は、見つかっていないモノの案内板が表示されている場合に、この変数に指定したトリガーがオーバーラップした時に、見つかっていないモノの案内板が非表示されます。  
この処理の主な目的は、引き返した際に新たにループ用に生成されるレベル上の見つかっていないモノの案内板が近くに表示されてしまうことへの対応です（特に大きな問題はありませんが、レベルの配置・構造によっては同じ位置から振り返るだけでもう１つの見つかっていないモノの案内板が見えてしまいます）。  

この見つかっていないモノのテキストは、見つかっていないモノの数に変化があった場合に自動的に切り替わります。  
表示位置によっては引き返す時にその変化の瞬間が視認できたり、HideTriggers配列変数で指定しているトリガーの場所によっては非表示になる瞬間が視認できたりする可能性があります。 
変化の瞬間や変化そのものが異変と勘違いされる可能性もあるため、見つかっていないモノの案内板の表示位置や、HideTriggers配列の指定の有無・トリガー位置に注意してください。  

なおHideTriggers配列変数をクリアしても内部的に参照・キャッシュされているのか常にHideTriggers配列変数に指定していたものが有効になってしまう現象を確認しています。  
このような場合は、レベル上のOsgs_BP_Actor_NotFounds_3DTextアクターブループリントを一旦削除し、再度Osgs_BP_Actor_NotFounds_3DTextアクターブループリントを同じ場所に配置してみてください。もしくはHideTriggers配列変数で指定していたトリガー（TriggerBox_HideNotFoundGuide）を削除してみてください。  
Findloople ver.1.1.2で参照・キャッシュをリセットする目的でOsgs_BP_Actor_NotFounds_3DTextアクターブループリントを配置し直しましたが、同様の現象が発生する場合は再配置を試してください。  
